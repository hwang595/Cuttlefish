...
def main():
    ...
    model = resnet.__dict__[args.arch](num_classes=_num_classes)
    ...
    if args.data in ("cifar10", "cifar100"): 
        ...
        train_loader = torch.utils.data.DataLoader(
            data_obj(root='../data', train=True, transform=transforms.Compose([
        ...
        training_set = data_obj(root='../data', split="train",
    ...
    if args.data in ("cifar10", "cifar100"): 
        ...
        val_loader = torch.utils.data.DataLoader(
            data_obj(root='../data', train=False, transform=transforms.Compose([
                transforms.ToTensor(),
                normalize,
            ])),
        ...
        testset = data_obj(root='../data', split="test",
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                        milestones=[milestone_epochs[0]-args.lr_warmup_epochs-1, milestone_epochs[1]-args.lr_warmup_epochs-1],
                                                        gamma=0.1)
    scheduler_warmup = GradualWarmupScheduler(optimizer, multiplier=args.scale_factor, 
                                            total_epoch=args.lr_warmup_epochs, 
                                            after_scheduler=lr_scheduler)

    if args.arch in ['resnet1202', 'resnet110', 'resnet56']:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc, loss, epoch_time = train(train_loader, model, criterion, optimizer, epoch,
        ...
        running_stats["Best-Val-Acc"] = best_prec1
        ...
        logger.info("@ Epoch: {}, compute time cost: {}".format(epoch, epoch_time))
...
def train(train_loader, model, criterion, optimizer, epoch, frobnorms=[], normalize=None, **kwargs):
    ...
    return top1.avg, losses.avg, epoch_timer
