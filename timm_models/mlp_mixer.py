...
from .layers import PatchEmbed, Mlp, LowrankMlp, AdaptMlp, GluMlp, GatedMlp, DropPath, lecun_normal_, to_2tuple
...
default_cfgs = dict(
    ...
    resmlp_36_224=_cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
    resmlp_36_224_pro=_cfg(
        url='https://dl.fbaipublicfiles.com/deit/resmlp_36_no_dist.pth',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
    lowrank_resmlp_36_224=_cfg(
        url='https://dl.fbaipublicfiles.com/deit/resmlp_36_no_dist.pth',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
    adapt_resmlp_36_224=_cfg(
        url='https://dl.fbaipublicfiles.com/deit/resmlp_36_no_dist.pth',
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
...
class LowrankResBlock(nn.Module):
    ...
    """
    def __init__(
            self, dim, seq_len, mlp_ratio=4, mlp_layer=LowrankMlp, norm_layer=Affine,
            act_layer=nn.GELU, init_values=1e-4, drop=0., drop_path=0.):
        super().__init__()
        channel_dim = int(dim * mlp_ratio)
        self.norm1 = norm_layer(dim)
...
class AdaptResBlock(nn.Module):
    """ Residual MLP block w/ LayerScale and Affine 'norm'

    Based on: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    def __init__(
            self, dim, seq_len, mlp_ratio=4, mlp_layer=AdaptMlp, norm_layer=Affine,
            act_layer=nn.GELU, init_values=1e-4, drop=0., drop_path=0., est_rank=None):
        super().__init__()
        channel_dim = int(dim * mlp_ratio)
        self.norm1 = norm_layer(dim)
        self.linear_tokens = nn.Linear(seq_len, seq_len)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        self.mlp_channels = mlp_layer(dim, channel_dim, act_layer=act_layer, drop=drop, est_rank=est_rank)
        self.ls1 = nn.Parameter(init_values * torch.ones(dim))
        self.ls2 = nn.Parameter(init_values * torch.ones(dim))
...
class LowrankMlpMixer(nn.Module):
    def __init__(
            self,
            num_classes=1000,
            img_size=224,
            ...
            embed_dim=512,
            mlp_ratio=(0.5, 4.0),
            vanilla_block_layer=ResBlock,
            block_layer=MixerBlock,
            vanilla_mlp_layer=Mlp,
            mlp_layer=LowrankMlp,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            act_layer=nn.GELU,
            drop_rate=0.,
            drop_path_rate=0.,
        ...
        for block_index in range(num_blocks):
            if block_index < 17:
                tempt_blocks.append(
                    vanilla_block_layer(
                           embed_dim, self.stem.num_patches, mlp_ratio, 
                           mlp_layer=vanilla_mlp_layer, 
        ...
        for block_index in range(num_blocks):
            tempt_blocks.append(
                block_layer(
                       ...
                       drop=drop_rate, 
                       drop_path=drop_path_rate,
                       est_rank=self._est_rank[self._rank_counter:self._rank_counter+2]
                       )
                )
...
def _create_mixer(variant, pretrained=False, est_rank=None, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for MLP-Mixer models.')

    if "adapt" in variant:
        model = build_model_with_cfg(
            AdaptMlpMixer, variant, pretrained,
            default_cfg=default_cfgs[variant],
            pretrained_filter_fn=checkpoint_filter_fn,
            est_rank=est_rank,
            **kwargs)
    elif "lowrank" in variant:
        model = build_model_with_cfg(
            LowrankMlpMixer, variant, pretrained,
            default_cfg=default_cfgs[variant],
            pretrained_filter_fn=checkpoint_filter_fn,
            **kwargs)
    else:
...
@register_model
def resmlp_12_224_pro(pretrained=False, **kwargs):
    """ ResMLP-12
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=12, embed_dim=768, mlp_ratio=4, block_layer=ResBlock, norm_layer=Affine, **kwargs)
    model = _create_mixer('resmlp_12_224_pro', pretrained=pretrained, **model_args)
    return model
...
@register_model
def resmlp_24_224_pro(pretrained=False, **kwargs):
    """ ResMLP-24
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=24, embed_dim=784, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-5), norm_layer=Affine, **kwargs)
    model = _create_mixer('resmlp_24_224_pro', pretrained=pretrained, **model_args)
    return model
...
@register_model
def lowrank_resmlp_36_224(pretrained=False, **kwargs):
    """ Pufferfish lowrank version of ResMLP-36
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=36, embed_dim=384, mlp_ratio=4,
        vanilla_block_layer=partial(ResBlock, init_values=1e-6),
        block_layer=partial(LowrankResBlock, init_values=1e-6), norm_layer=Affine, **kwargs)
    model = _create_mixer('lowrank_resmlp_36_224', pretrained=pretrained, **model_args)
    return model


@register_model
def adapt_resmlp_36_224(pretrained=False, **kwargs):
    """ Adaptive lowrank version of ResMLP-36
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=36, embed_dim=384, mlp_ratio=4,
        block_layer=partial(AdaptResBlock, init_values=1e-6), norm_layer=Affine, **kwargs)
    model = _create_mixer('adapt_resmlp_36_224', pretrained=pretrained, **model_args)
    return model


@register_model
def resmlp_36_224_pro(pretrained=False, **kwargs):
    """ ResMLP-36
    Paper: `ResMLP: Feedforward networks for image classification...` - https://arxiv.org/abs/2105.03404
    """
    model_args = dict(
        patch_size=16, num_blocks=36, embed_dim=784, mlp_ratio=4,
        block_layer=partial(ResBlock, init_values=1e-6), norm_layer=Affine, **kwargs)
    model = _create_mixer('resmlp_36_224_pro', pretrained=pretrained, **model_args)
    return model
